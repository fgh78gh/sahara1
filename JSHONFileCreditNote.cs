...
{
    public class JSHONFileCreditNote
    {
        ...
        public string VersionValue(String strInvoiceNo)
        {
            ...
            StrQry = "set dateformat dmy;select 'B2B' InvType ,CreditNoteNo INVNO,CONVERT(VARCHAR(10),CreditNoteDate,103) INVDT,tblCustomerMaster.GSTNO Buyergstno,name Buyername,StateCode BuyerPOS,"
                      + " Address1 BuyerAdd1, (Address2 +' '+  Address3) BuyerAdd2,StateName BuyerLocation,PinCode BuyerPinCode,StateCode BuyerStateCode,"
                      + " CompanyPhoneNo BuyerPhoneNo,CompanyEmail BuyerEmailId,tblCustomerMaster.GSTNO ShipGSTNo,Name ShipName,Address1 ShipAddr1,"
                      + " (Address2 +' '+  Address3) ShipAddr2,StateName ShipLoc,PinCode ShipPinCode,StateCode ShipStateCode, "
                      + " TotalTaxableValue TotalTaxableValue,CGST,SGST,IGST,RoundOff,TCSValue OthChrg,InvoiceValue InvoiceValue,"
                      + " (CASE WHEN ((select InvoiceFor from tblinvoice where tblinvoice.invoiceno=tblCreditNote.InvoiceNo)='GOODS') THEN 'N' ELSE 'Y'END) InvoiceFor from "
                      + " tblCreditNote INNER JOIN tblCustomerMaster ON tblCreditNote.CustomerId = tblCustomerMaster.id where CreditNoteNo='" + strInvoiceNo + "'";
        ...
        public List<ItemList> ItemListDataA(String InvoiceNo,String InvoiceFor)
        {
            ...
            String StrQry = "select ROW_NUMBER()over(order by ItemCode) PrdSlNo, TotalValue TotItemVal,SGSTValue SgstAmt,CGSTValue CgstAmt,IGSTValue IgstAmt,Delqty Qty,TaxableValue TaxableValue,TaxableValue TotalTaxableValue,UnitPrice UnitPrice,UOM Unit,"
                    ...
                    + " from tblCreditNoteItem where CreditNoteid in (Select tblCreditNote.id from tblCreditNote where CreditNoteNo='" + InvoiceNo + "')";
