...
{
    public class JSHONFileExportInvoice
    {
        ...
        public string VersionValue(String strInvoiceNo)
        {
            ...
            StrQry = "set dateformat dmy;select 'EXPWOP' InvType ,InvoiceNo INVNO,CONVERT(VARCHAR(10),InvoiceDate,103) INVDT,'URP' Buyergstno,name Buyername,'96' BuyerPOS,"
                      + " Address1 BuyerAdd1, (Address2 +' '+  Address3) BuyerAdd2,StateName BuyerLocation,'96' BuyerStateCode,"
                      + " CompanyPhoneNo BuyerPhoneNo,CompanyEmail BuyerEmailId,'URP' ShipGSTNo,Shipname ShipName,ShipAddress1 ShipAddr1,"
                      + " (ShipAddress2 +' '+  ShipAddress3) ShipAddr2,ShipState ShipLoc,'96' ShipStateCode, "
                      + " (Select sum(TotalPriceinUSD) from tblinvoiceitem where invoiceid=tblInvoice.id) TotalTaxableValue,CGST,SGST,IGST,RoundOff,TCSValue OthChrg,(Select sum(TotalPriceinUSD) from tblinvoiceitem where invoiceid=tblInvoice.id) InvoiceValue,"
                      + " (CASE WHEN (InvoiceFor='GOODS') THEN 'N' ELSE 'Y'END)InvoiceFor from "
                      + " tblInvoice INNER JOIN tblCustomerMaster ON tblInvoice.CustomerId = tblCustomerMaster.id where tblInvoice.deleted=0 and invoiceno='" + strInvoiceNo + "'";
            ...
            Root root = new Root()
            {
                ...
                BuyerDtls = new BuyerDtls()
                {
                    ...
                    Ph = dsData.Tables["tblInvoice"].Rows[0]["BuyerPhoneNo"].ToString(),
                    Em = dsData.Tables["tblInvoice"].Rows[0]["BuyerEmailId"].ToString(),
                },
        ...
        {
            DataSet dsData = new DataSet();
            String StrQry = "select ROW_NUMBER()over(order by ItemCode) PrdSlNo, TotalPriceinUSD TotItemVal,SGSTValue SgstAmt,CGSTValue CgstAmt,IGSTValue IgstAmt,Delqty Qty,TotalPriceinUSD TaxableValue,TotalPriceinUSD TotalTaxableValue,UnitPriceinUSD UnitPrice,UOM Unit,"
                    + " (CGSTPercent+SGSTPercent+IGSTPercent) GSTPercent,HSNNo HSNCode,ItemName ProductDescription "
                    + " from tblinvoiceitem where invoiceid in (Select tblinvoice.id from tblinvoice where deleted=0 and invoiceno='" + InvoiceNo + "')";
