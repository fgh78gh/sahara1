...
namespace SaharaLabel
{
    public class JSHONFile
    {
        ...
        public string VersionValue1(String strInvoiceNo)
        {
            ...
            StrQry = "set dateformat dmy;select (CASE WHEN (TransType='SEZ') THEN 'SEZWOP' WHEN (TransType='EXPORT') THEN 'EXPWOP' ELSE 'B2B'END)InvType ,InvoiceNo INVNO,CONVERT(VARCHAR(10),InvoiceDate,103) INVDT,tblCustomerMaster.GSTNO Buyergstno,name Buyername,StateCode BuyerPOS,"
                      + " Address1 BuyerAdd1, (Address2 +' '+  Address3) BuyerAdd2,StateName BuyerLocation,PinCode BuyerPinCode,StateCode BuyerStateCode,"
                      + " CompanyPhoneNo BuyerPhoneNo,CompanyEmail BuyerEmailId,ShipGSTNo ShipGSTNo,Shipname ShipName,ShipAddress1 ShipAddr1,"
                      + " (ShipAddress2 +' '+  ShipAddress3) ShipAddr2,ShipState ShipLoc,shippincode ShipPinCode,ShipStateCode ShipStateCode, "
                      + " TotalTaxableValue TotalTaxableValue,CGST,SGST,IGST,RoundOff,TCSValue OthChrg,InvoiceValue InvoiceValue,"
                      + " (CASE WHEN (InvoiceFor='GOODS') THEN 'N' ELSE 'Y'END)InvoiceFor,tblInvoice.Distance,tblInvoice.VechileNo from "
